...
Version 2.7.0 adds a public, search-friendly Wealth Wars website at the root domain and moves the playable multiplayer experience to `/play`. The homepage explains the behavioral-investing concept, long-term strategy, social-media influence connection, rules, learning outcomes and frequently asked questions. It also includes search metadata, structured site data, a sitemap, robots instructions and a branded social-sharing image. Every Play button leads to the existing game without changing its Socket.IO multiplayer connection.

The always-available Player Handbook remains available before and during a match. It explains the purpose, winning score, full turn flow, level unlocks and every major control. A searchable plain-language glossary defines investing concepts, cognitive biases and game-only perks without interrupting play.
