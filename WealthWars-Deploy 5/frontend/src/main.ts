...
let lastNotifiedRollId = '';
let lastNotifiedCardKey = '';
let lastKnownLevel = 1;
let lastKnownInvestorProgress: number | null = null;
let investmentOrderSide: 'buy' | 'sell' = 'buy';
...
const INVESTMENT_CATALOG: Record<string, { name: string; type: 'stock' | 'fund' | 'bond' | 'property'; minLevel: number }> = {
  CONS: { name: 'Balanced Index Fund', type: 'fund', minLevel: 1 },
  TECH: { name: 'Nova Tech', type: 'stock', minLevel: 1 },
  HEAL: { name: 'Vitalis Health', type: 'stock', minLevel: 1 },
  ENER: { name: 'Green Grid Energy', type: 'stock', minLevel: 2 },
  BOND: { name: 'Government Bond', type: 'bond', minLevel: 2 },
  PROP: { name: 'Harbor Studio', type: 'property', minLevel: 3 }
};

const TILE_TIPS: Record<string, string> = {
  Income: 'Fresh cash becomes powerful when part of it is consistently allocated to long-term assets.',
  Decision: 'Choose the strategy that fits your horizon, diversification and capacity for loss.',
  Investment: 'Research-edge tiles provide special pricing or risk-management abilities—not ordinary purchases.',
  Market: 'Market events change prices for everyone. A long horizon still requires diversification and liquidity.',
  LifeEvent: 'Portfolio administration, fees, taxes and distributions all affect long-term returns.',
  Opportunity: 'A good opportunity improves a sound plan; it should not replace one.'
};

type GlossaryTerm = { term: string; definition: string; tags: string };

const GLOSSARY_TERMS: GlossaryTerm[] = [
  { term: 'Asset Allocation', definition: 'How your money is divided among cash, stocks, bonds and real estate. The mix determines both growth potential and risk.', tags: 'portfolio diversification mix' },
  { term: 'Bond', definition: 'A loan you make to a government or company. In the game, bonds unlock at Level 2 and provide steadier returns than stocks.', tags: 'fixed income level 2' },
  { term: 'Cash', definition: 'Money you can spend immediately. Cash is useful for opportunities, but too much cash may grow more slowly than invested money.', tags: 'liquidity money' },
  { term: 'Cognitive Bias', definition: 'A mental shortcut that can distort a decision. Wealth Wars questions reveal biases and explain how they can affect investing.', tags: 'decision psychology' },
  { term: 'Concentration Risk', definition: 'The danger of having too much money in one investment or sector. Diversification reduces this risk.', tags: 'diversification portfolio' },
  { term: 'Confirmation Bias', definition: 'Looking mainly for information that supports what you already believe while ignoring evidence that challenges it.', tags: 'cognitive social media' },
  { term: 'Cost Basis', definition: 'What you originally paid for an investment. The game compares cost basis with the current value to calculate your gain or loss.', tags: 'return purchase price' },
  { term: 'Debt', definition: 'Money you owe. Debt lowers financial resilience and may create future payments through Decision Ripples.', tags: 'loan liability health' },
  { term: 'Decision Ripple', definition: 'A future payment or reward caused by an earlier choice. Open the Ripples tab to see what is scheduled.', tags: 'future commitment consequence' },
  { term: 'Diversification', definition: 'Spreading money across different assets so one bad result does not damage the whole portfolio.', tags: 'risk allocation' },
  { term: 'Downside Shield', definition: 'A game-only perk that absorbs or reduces one eligible future loss. It is used automatically when the matching loss occurs.', tags: 'perk protection loss' },
  { term: 'Emergency Fund', definition: 'Cash set aside for unexpected costs. It supports your Financial Health Score but is not available for investing until withdrawn.', tags: 'savings liquidity resilience' },
  { term: 'Financial Health Score', definition: 'The final winning measure. Net worth matters most, with support from liquidity, manageable debt and diversification.', tags: 'win score victory' },
  { term: 'FOMO', definition: 'Fear of missing out: buying because other people appear to be profiting, instead of checking price, evidence and fit.', tags: 'bias social media hype' },
  { term: 'Herding', definition: 'Following the crowd because many people are doing the same thing, even when the evidence is weak.', tags: 'bias social influence' },
  { term: 'Hype Pull', definition: 'Your private in-game indicator of how strongly exciting social-media claims influence your choices.', tags: 'pattern social media' },
  { term: 'Independent Judgment', definition: 'Your private in-game indicator of how often you form a decision from evidence and your own plan rather than crowd pressure.', tags: 'pattern evidence' },
  { term: 'Index Fund', definition: 'A collection of many investments designed to track a market group. It can provide instant diversification.', tags: 'stock fund diversification' },
  { term: 'Inflation', definition: 'A broad rise in prices that reduces what money can buy. In the game, inflation is part of the market environment.', tags: 'economy purchasing power' },
  { term: 'Insider Trading', definition: 'Trading using material information that is not public. It is illegal in real life. Fictional insider cards exist only as a game mechanic and are not real-world advice.', tags: 'illegal disclaimer inside information' },
  { term: 'Investor Progress', definition: 'Progress earned by making well-reasoned decisions. Reach the target to unlock the option to advance to a higher level.', tags: 'level unlock advancement' },
  { term: 'Liquidity', definition: 'How easily you can access money for near-term needs without selling long-term assets at a bad time.', tags: 'cash emergency fund' },
  { term: 'Loss Aversion', definition: 'Feeling losses more strongly than equivalent gains, which can lead to panic-selling or refusing to correct a weak investment.', tags: 'bias psychology' },
  { term: 'Market Event', definition: 'A game event that changes investment prices for every player. It tests how prepared and diversified each portfolio is.', tags: 'price economy tile' },
  { term: 'Net Worth', definition: 'Everything you own in cash and investments, minus debt. It is a major part of your Financial Health Score.', tags: 'wealth assets liabilities' },
  { term: 'Portfolio', definition: 'All the investments and cash you own together.', tags: 'holdings assets' },
  { term: 'Real Estate', definition: 'Property as an investment. It unlocks at Level 3 and can provide income, but may also create maintenance costs.', tags: 'property level 3' },
  { term: 'Recency Bias', definition: 'Assuming what happened most recently will continue, even when long-term evidence says otherwise.', tags: 'bias performance' },
  { term: 'Research Discount', definition: 'A game-only perk that lowers the cost of an eligible purchase because you landed on a research opportunity.', tags: 'perk investment offer' },
  { term: 'Stock', definition: 'A small ownership share in a company. Its price can rise or fall as the market changes.', tags: 'equity share' },
  { term: 'Unrealized Gain/Loss', definition: 'The profit or loss on an investment you still own. It becomes realized only when you sell.', tags: 'return position percentage' },
  { term: 'Verification Habit', definition: 'Your private in-game indicator of how often you check evidence before acting on an investment claim.', tags: 'pattern research social media' },
  { term: 'Volatility', definition: 'How sharply and often an investment price moves. Higher volatility creates larger possible gains and losses.', tags: 'risk price movement' }
];

function guideMarkup(surfaceId: string) {
  const glossary = GLOSSARY_TERMS.map(item => `
    <article class="glossary-card" data-glossary-search="${item.term.toLowerCase()} ${item.tags}">
      <h4>${item.term}</h4>
      <p>${item.definition}</p>
    </article>`).join('');

  return `
    <div class="guide-nav" role="tablist" aria-label="Game information sections">
      <button class="guide-nav-btn active" type="button" data-guide-target="purpose" role="tab">Purpose</button>
      <button class="guide-nav-btn" type="button" data-guide-target="play" role="tab">How to Play</button>
      <button class="guide-nav-btn" type="button" data-guide-target="controls" role="tab">Buttons</button>
      <button class="guide-nav-btn" type="button" data-guide-target="glossary" role="tab">Glossary</button>
    </div>

    <section class="guide-panel active" data-guide-panel="purpose">
      <div class="guide-hero">
        <span class="guide-kicker">THE PURPOSE</span>
        <h3>Build wealth that can survive pressure</h3>
        <p>Wealth Wars is a 2–4 player, 20-round investing strategy game. You balance long-term growth, available cash, debt and diversification while markets move and social-media claims test your judgment.</p>
      </div>
      <div class="guide-callout"><strong>How you win</strong><span>The player with the highest <b>Financial Health Score</b> after Round 20 wins. Net worth counts most—but resilience, liquidity, healthy debt and diversification can decide a close game.</span></div>
      <div class="guide-card-grid">
        <article><span>01</span><h4>Invest</h4><p>Buy and sell stocks, funds, bonds and real estate as levels unlock.</p></article>
        <article><span>02</span><h4>Judge evidence</h4><p>Answer clear investing scenarios and learn which cognitive bias a weak choice reflects.</p></article>
        <article><span>03</span><h4>Read your pattern</h4><p>Your private indicators reveal how verification, hype and independent judgment shape your decisions.</p></article>
      </div>
      <p class="guide-disclaimer"><strong>Educational game:</strong> Wealth Wars is not financial advice. Fictional inside-information cards are game mechanics; insider trading is illegal in real life.</p>
    </section>

    <section class="guide-panel" data-guide-panel="play">
      <div class="guide-section-heading"><span class="guide-kicker">ONE COMPLETE TURN</span><h3>Roll, respond, invest, end</h3></div>
      <ol class="turn-flow-list">
        <li><span>1</span><div><strong>Prepare the room</strong><p>Enter a name, create or join a boardroom, then Mark Ready. The host launches when 2–4 players are ready.</p></div></li>
        <li><span>2</span><div><strong>Start your turn</strong><p>Your level-based income arrives. When your name appears in the turn indicator, press Roll Dice.</p></div></li>
        <li><span>3</span><div><strong>Resolve the tile</strong><p>Read the card, choose an answer or claim/pass on an offer. Market events may affect everyone.</p></div></li>
        <li><span>4</span><div><strong>Manage your portfolio</strong><p>Use Invest Anytime to buy or sell eligible assets. You can also move $100 into or out of your emergency fund.</p></div></li>
        <li><span>5</span><div><strong>End the turn</strong><p>Check your Portfolio and Ripples, then press End Turn so the next player can act.</p></div></li>
      </ol>
      <div class="level-roadmap">
        <div><b>Level 1</b><span>Stocks, balanced fund and cash</span></div>
        <div><b>Level 2</b><span>Bonds and energy investment unlock</span></div>
        <div><b>Level 3</b><span>Real estate and advanced choices unlock</span></div>
      </div>
    </section>

    <section class="guide-panel" data-guide-panel="controls">
      <div class="guide-section-heading"><span class="guide-kicker">CONTROL DIRECTORY</span><h3>What every button does</h3></div>
      <div class="control-groups">
        <article><h4>Lobby</h4><dl><div><dt>Create Boardroom</dt><dd>Starts a new room that others can join.</dd></div><div><dt>Join Room</dt><dd>Takes an available seat in that room.</dd></div><div><dt>Mark Ready</dt><dd>Confirms you are ready to begin.</dd></div><div><dt>Launch Boardroom</dt><dd>Host-only: starts after at least two players are ready.</dd></div></dl></article>
        <article><h4>Your turn</h4><dl><div><dt>Roll Dice</dt><dd>Moves your token and reveals the tile outcome.</dd></div><div><dt>Continue / Pass</dt><dd>Resolves or declines the card currently shown.</dd></div><div><dt>End Turn</dt><dd>Hands control to the next player after all required choices are complete.</dd></div></dl></article>
        <article><h4>Money</h4><dl><div><dt>Buy / Sell</dt><dd>Chooses the direction of your next order.</dd></div><div><dt>Buy Now / Sell Now</dt><dd>Executes the selected asset and quantity at the shown estimate.</dd></div><div><dt>Save / Withdraw $100</dt><dd>Moves money between liquid cash and your emergency fund.</dd></div></dl></article>
        <article><h4>Information</h4><dl><div><dt>Portfolio</dt><dd>Shows net worth, health, holdings and live gains/losses.</dd></div><div><dt>Ripples</dt><dd>Shows scheduled future costs and rewards.</dd></div><div><dt>Activity</dt><dd>Shows recent public game and market events.</dd></div><div><dt>Guide / ?</dt><dd>Opens these rules and the searchable glossary.</dd></div></dl></article>
      </div>
    </section>

    <section class="guide-panel" data-guide-panel="glossary">
      <div class="guide-section-heading"><span class="guide-kicker">PLAIN-LANGUAGE FINANCE</span><h3>Search the glossary</h3><p>Type a term, topic or idea—such as “risk,” “bias” or “cash.”</p></div>
      <label class="glossary-search-wrap" for="${surfaceId}-glossary-search"><span aria-hidden="true">⌕</span><input id="${surfaceId}-glossary-search" class="glossary-search" type="search" placeholder="Search finance terms..." autocomplete="off" /></label>
      <div class="glossary-count" aria-live="polite">${GLOSSARY_TERMS.length} terms</div>
      <div class="glossary-grid">${glossary}</div>
      <p class="glossary-empty hidden">No matching term yet. Try a broader word.</p>
    </section>`;
}

function bindGuideSurface(container: HTMLElement, surfaceId: string) {
  container.innerHTML = guideMarkup(surfaceId);
  const navButtons = Array.from(container.querySelectorAll<HTMLButtonElement>('.guide-nav-btn'));
  const panels = Array.from(container.querySelectorAll<HTMLElement>('.guide-panel'));
  navButtons.forEach(button => button.addEventListener('click', () => {
    navButtons.forEach(item => item.classList.remove('active'));
    panels.forEach(panel => panel.classList.remove('active'));
    button.classList.add('active');
    container.querySelector<HTMLElement>(`[data-guide-panel="${button.dataset.guideTarget}"]`)?.classList.add('active');
    sounds.playClick();
  }));

  const search = container.querySelector<HTMLInputElement>('.glossary-search');
  const cards = Array.from(container.querySelectorAll<HTMLElement>('.glossary-card'));
  const count = container.querySelector<HTMLElement>('.glossary-count');
  const empty = container.querySelector<HTMLElement>('.glossary-empty');
  search?.addEventListener('input', () => {
    const query = search.value.trim().toLowerCase();
    let visible = 0;
    cards.forEach(card => {
      const matches = !query || (card.dataset.glossarySearch || '').includes(query) || (card.textContent || '').toLowerCase().includes(query);
...
const btnGameGuide = document.getElementById('btn-game-guide') as HTMLButtonElement;
...
const lblInfluencePattern = document.getElementById('lbl-influence-pattern')!;
const lblInfluenceInsight = document.getElementById('lbl-influence-insight')!;
const lblInfluenceVerification = document.getElementById('lbl-influence-verification')!;
const lblInfluenceHype = document.getElementById('lbl-influence-hype')!;
const lblInfluenceIndependent = document.getElementById('lbl-influence-independent')!;
const barInfluenceVerification = document.getElementById('bar-influence-verification')!;
const barInfluenceHype = document.getElementById('bar-influence-hype')!;
const barInfluenceIndependent = document.getElementById('bar-influence-independent')!;
const lblInfluenceSample = document.getElementById('lbl-influence-sample')!;
const investmentAssetSelect = document.getElementById('investment-asset-select') as HTMLSelectElement;
const investmentQuantity = document.getElementById('investment-quantity') as HTMLInputElement;
const btnOrderBuy = document.getElementById('btn-order-buy') as HTMLButtonElement;
const btnOrderSell = document.getElementById('btn-order-sell') as HTMLButtonElement;
const investmentEstimateLabel = document.getElementById('investment-estimate-label')!;
const investmentEstimatedCost = document.getElementById('investment-estimated-cost')!;
const investmentPerkStatus = document.getElementById('investment-perk-status')!;
const investmentOrderNote = document.getElementById('investment-order-note')!;
const btnInvestAnytime = document.getElementById('btn-invest-anytime') as HTMLButtonElement;
...
const modalDecisionFeedback = document.getElementById('modal-decision-feedback')!;
const decisionFeedbackCard = document.getElementById('decision-feedback-card')!;
const decisionFeedbackOutcome = document.getElementById('decision-feedback-outcome')!;
const decisionFeedbackTitle = document.getElementById('decision-feedback-title')!;
const decisionFeedbackBias = document.getElementById('decision-feedback-bias')!;
const decisionFeedbackExplanation = document.getElementById('decision-feedback-explanation')!;
const decisionFeedbackImpact = document.getElementById('decision-feedback-impact')!;
const btnDecisionFeedbackContinue = document.getElementById('btn-decision-feedback-continue') as HTMLButtonElement;
const modalGameGuide = document.getElementById('modal-game-guide')!;
const btnGuideClose = document.getElementById('btn-guide-close') as HTMLButtonElement;
const guideModalContent = document.getElementById('guide-modal-content')!;
const informationTabContent = document.getElementById('information-tab-content')!;
...
function notifyGameMoments(state: any) {
  const me = state.players.find((player: any) => player.id === myId);
  if (!me) return;

  if (me.level > lastKnownLevel) {
    const unlocked = me.level === 2
      ? 'Bonds, energy equities and Level 2 strategy questions are now available. You also earned a 15% research discount.'
      : 'Real estate and advanced allocation questions are now available. You also earned a 20% research discount and a downside shield.';
    showToast(`Level ${me.level} Investing Unlocked`, unlocked, 'income', '★');
  }
  lastKnownLevel = me.level;

  if (lastKnownInvestorProgress !== null && me.investorProgress > lastKnownInvestorProgress) {
    const gained = me.investorProgress - lastKnownInvestorProgress;
    showToast(
      `Investor Progress +${gained}`,
      `You now have ${me.investorProgress} mastery point${me.investorProgress === 1 ? '' : 's'}. Strong decisions and one-time portfolio milestones unlock new levels.`,
      'income',
      '◆'
    );
  }
  lastKnownInvestorProgress = me.investorProgress;

  const pending = state.currentPendingAction;
  const card = pending?.details?.card;
  const event = pending?.details?.event;
  const cardKey = card
    ? `${state.currentRound}:${pending.initiatorId}:${card.id}`
    : event
      ? `${state.currentRound}:${pending.initiatorId}:${event.id}`
      : '';
  if (cardKey && cardKey !== lastNotifiedCardKey) {
    lastNotifiedCardKey = cardKey;
    if (card) {
      showToast(
        card.gameOnlyInsiderMechanic ? 'Game-Only Insider Card' : card.title,
        card.tip,
        card.gameOnlyInsiderMechanic ? 'event' : 'info',
        card.gameOnlyInsiderMechanic ? '⚖️' : '💡'
      );
    } else if (event) {
      showToast(event.name, `${event.description} Watch how each asset responds.`, 'event', '⚡');
    }
    return;
  }

  const roll = state.lastRoll;
  if (roll && roll.rollId !== lastNotifiedRollId) {
    lastNotifiedRollId = roll.rollId;
    const roller = state.players.find((player: any) => player.id === roll.playerId);
    if (!roller) return;
    const tile = BOARD_TILES[roller.level]?.[roller.position];
    const tip = TILE_TIPS[tile];
    if (tip) showToast(`${roller.name} · ${tile}`, tip, tile === 'Income' ? 'income' : 'info', '💡');
  }
}
...
function init() {
  ...
  updateSoundUI();
  bindGuideSurface(guideModalContent, 'modal-guide');
  bindGuideSurface(informationTabContent, 'panel-guide');
  ...
  socket.on('game_started', (gameState: any) => {
    ...
    lastKnownLevel = 1;
    lastKnownInvestorProgress = null;
  ...
  socket.on('investment_confirmation', (result: any) => {
    showToast('Investment Added', result.message, 'income', '↗');
  });

  socket.on('sale_confirmation', (result: any) => {
    showToast('Holding Sold', result.message, 'income', '⇄');
  });

  socket.on('decision_feedback', (feedback: any) => {
    const outcomeLabels: Record<string, string> = {
      optimal: 'STRONGEST FIT',
      reasonable: 'DEFENSIBLE CHOICE',
      costly: 'BIAS TRAP'
    };
    decisionFeedbackCard.className = `decision-feedback-card outcome-${feedback.outcome}`;
    decisionFeedbackOutcome.textContent = outcomeLabels[feedback.outcome] || 'DECISION REVIEW';
    decisionFeedbackTitle.textContent = feedback.choiceLabel;
    decisionFeedbackExplanation.textContent = feedback.explanation;
    const progressResult = feedback.isScoredDecision
      ? feedback.progressEarned > 0
        ? ` · +${feedback.progressEarned} Investor Progress (${feedback.progressTotal}/${feedback.progressTarget})`
        : ` · No progress awarded (${feedback.progressTotal}/${feedback.progressTarget})`
      : '';
    const cashResult = feedback.cashBonus > 0 ? ` · +$${feedback.cashBonus} strategy bonus` : '';
    decisionFeedbackImpact.textContent = `${feedback.impact}${cashResult}${progressResult}`;
    if (feedback.bias) {
      decisionFeedbackBias.textContent = `Bias identified: ${feedback.bias}`;
      decisionFeedbackBias.classList.remove('hidden');
    } else {
      decisionFeedbackBias.classList.add('hidden');
    }
    if (feedback.socialInfluenceUpdate) {
      showToast(
        `Private pattern: ${feedback.socialInfluenceUpdate.patternTitle}`,
        feedback.socialInfluenceUpdate.lastInsight,
        'event',
        '◉'
      );
    }
    modalDecisionFeedback.classList.add('active');
  });
...
function openGuideModal() {
  modalGameGuide.classList.add('active');
  document.body.classList.add('guide-open');
  btnGuideClose.focus();
}

function closeGuideModal() {
  modalGameGuide.classList.remove('active');
  document.body.classList.remove('guide-open');
}

function activatePersonalPanelTab(targetId: string) {
...
function bindEvents() {
  document.querySelectorAll<HTMLButtonElement>('.btn-open-guide').forEach(button => {
  ...
  modalGameGuide.addEventListener('click', event => {
    if (event.target === modalGameGuide) closeGuideModal();
  });

  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && modalGameGuide.classList.contains('active')) closeGuideModal();
  });

  btnGameGuide.addEventListener('click', () => {
    sounds.playClick();
    activatePersonalPanelTab('tab-information');
  });
  ...
  investmentAssetSelect.addEventListener('change', updateInvestmentOrderPreview);
  investmentQuantity.addEventListener('input', updateInvestmentOrderPreview);
  btnOrderBuy.addEventListener('click', () => {
    investmentOrderSide = 'buy';
    btnOrderBuy.classList.add('active');
    btnOrderSell.classList.remove('active');
    updateInvestmentOrderPreview();
  });
  btnOrderSell.addEventListener('click', () => {
    investmentOrderSide = 'sell';
    btnOrderSell.classList.add('active');
    btnOrderBuy.classList.remove('active');
    updateInvestmentOrderPreview();
  });
  btnInvestAnytime.addEventListener('click', () => {
    if (!activeLobbyId) return;
    const quantity = Math.max(1, Math.min(10, Number.parseInt(investmentQuantity.value || '1', 10)));
    sounds.playCash();
    socket.emit(investmentOrderSide === 'buy' ? 'invest_anytime' : 'sell_anytime', {
      gameId: activeLobbyId,
      assetSymbol: investmentAssetSelect.value,
      quantity
    });
  });

  btnDecisionFeedbackContinue.addEventListener('click', () => {
    modalDecisionFeedback.classList.remove('active');
    sounds.playClick();
  });
  ...
  document.querySelectorAll('.tab-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      activatePersonalPanelTab(targetId);
    });
  });
...
function updateLobbyRoom(room: any) {
  ...
  if (room.hostId === myId && room.players.length >= 2 && room.players.every((p: any) => p.ready && p.connected !== false)) {
...
function selectedAssetPrice(state: any, symbol: string): number {
  if (symbol === 'BOND') return state.marketPrices.bonds.BOND;
  if (symbol === 'PROP') return state.marketPrices.realEstate.PROP;
  return state.marketPrices.stocks[symbol] || 0;
}

function ownedAssetQuantity(player: any, symbol: string): number {
  if (symbol === 'BOND') return player.portfolio.bonds.length;
  if (symbol === 'PROP') return player.portfolio.properties.length;
  return player.portfolio.stocks[symbol] || 0;
}

function updateInvestmentOrderPreview() {
  if (!latestGameState) return;
  const me = latestGameState.players.find((player: any) => player.id === myId);
  if (!me) return;
  const symbol = investmentAssetSelect.value;
  const asset = INVESTMENT_CATALOG[symbol];
  const quantity = Math.max(1, Math.min(10, Number.parseInt(investmentQuantity.value || '1', 10)));
  investmentQuantity.value = `${quantity}`;
  const marketAmount = selectedAssetPrice(latestGameState, symbol) * quantity;
  const owned = ownedAssetQuantity(me, symbol);
  const isBuy = investmentOrderSide === 'buy';
  const discount = isBuy && me.investmentPerks?.discountUses > 0 ? me.investmentPerks.nextTradeDiscount : 0;
  const estimatedAmount = isBuy
    ? Math.max(1, Math.round(marketAmount * (1 - discount)))
    : Math.round(marketAmount * (asset.type === 'bond' ? 0.95 : asset.type === 'property' ? 0.90 : 1));
  const locked = isBuy && me.level < asset.minLevel;
  const hasEnough = isBuy ? me.cash >= estimatedAmount : owned >= quantity;

  Array.from(investmentAssetSelect.options).forEach(option => {
    const optionAsset = INVESTMENT_CATALOG[option.value];
    const optionOwned = ownedAssetQuantity(me, option.value);
    option.disabled = isBuy && me.level < optionAsset.minLevel;
    const suffix = isBuy
      ? option.disabled ? ' 🔒' : ''
      : ` · owned ${optionOwned}`;
    option.textContent = `${optionAsset.name} · $${selectedAssetPrice(latestGameState, option.value)}${suffix}`;
  });

  investmentEstimateLabel.textContent = isBuy ? 'Estimated cost' : 'Estimated proceeds';
  investmentEstimatedCost.textContent = `$${estimatedAmount.toLocaleString()}`;
  btnInvestAnytime.textContent = isBuy ? 'BUY NOW' : 'SELL NOW';
  btnInvestAnytime.classList.toggle('sell-order', !isBuy);
  btnInvestAnytime.disabled = locked || !hasEnough || latestGameState.status !== 'playing';

  if (locked) {
    investmentOrderNote.textContent = `${asset.name} unlocks at Level ${asset.minLevel}.`;
  } else if (isBuy && !hasEnough) {
    investmentOrderNote.textContent = `You need $${estimatedAmount.toLocaleString()} cash for this order.`;
  } else if (!isBuy && !hasEnough) {
    investmentOrderNote.textContent = `You own ${owned}; reduce the quantity to sell this asset.`;
  } else if (!isBuy && asset.type === 'bond') {
    investmentOrderNote.textContent = `Early bond sales receive 95% of face value. You own ${owned}.`;
  } else if (!isBuy && asset.type === 'property') {
    investmentOrderNote.textContent = `Property sales include 10% transaction costs. You own ${owned}.`;
  } else {
    investmentOrderNote.textContent = `${quantity} ${asset.type === 'stock' || asset.type === 'fund' ? 'share' : asset.type}${quantity === 1 ? '' : 's'} can be ${isBuy ? 'purchased' : 'sold'} now.`;
  }

  if (isBuy && discount > 0) {
    investmentPerkStatus.classList.add('active');
    investmentPerkStatus.textContent = `${Math.round(discount * 100)}% research discount · ${me.investmentPerks.discountUses} use${me.investmentPerks.discountUses === 1 ? '' : 's'} remaining · save $${(marketAmount - estimatedAmount).toLocaleString()}`;
  } else {
    investmentPerkStatus.classList.remove('active');
    investmentPerkStatus.textContent = me.investmentPerks?.downsideShieldUses > 0
      ? `${me.investmentPerks.downsideShieldUses} downside shield${me.investmentPerks.downsideShieldUses === 1 ? '' : 's'} ready`
      : 'No active research edge';
  }
}

function renderInvestmentDesk(me: any) {
  if (investmentOrderSide === 'buy' && me.level < INVESTMENT_CATALOG[investmentAssetSelect.value].minLevel) {
    investmentAssetSelect.value = 'CONS';
  }
  updateInvestmentOrderPreview();
}
      ...
      <div class="player-card-top">
        ...
        <div class="player-identity-wrap">
          ...
          <div class="player-card-sub">
            ...
            <span class="level-tag">LVL ${p.level} · ◆ ${p.investorProgress}</span>
          </div>
        </div>
...
function renderPersonalPanel(state: any, me: any) {
  ...
  const influenceProfile = me.socialInfluenceProfile || {
    decisionsTracked: 0,
    verification: 50,
    hypePull: 50,
    independentJudgment: 50,
    patternTitle: 'Pattern forming',
    lastInsight: 'Your private pattern develops when social-media investment claims appear.'
  };
  lblInfluencePattern.textContent = influenceProfile.patternTitle;
  lblInfluenceInsight.textContent = influenceProfile.lastInsight;
  lblInfluenceVerification.textContent = `${influenceProfile.verification}`;
  lblInfluenceHype.textContent = `${influenceProfile.hypePull}`;
  lblInfluenceIndependent.textContent = `${influenceProfile.independentJudgment}`;
  barInfluenceVerification.style.width = `${influenceProfile.verification}%`;
  barInfluenceHype.style.width = `${influenceProfile.hypePull}%`;
  barInfluenceIndependent.style.width = `${influenceProfile.independentJudgment}%`;
  lblInfluenceSample.textContent = influenceProfile.decisionsTracked === 0
    ? 'No social-media decisions encountered yet'
    : `${influenceProfile.decisionsTracked} social-media decision${influenceProfile.decisionsTracked === 1 ? '' : 's'} shaping this pattern`;
  ...
  const nextThreshold = me.level === 1 ? 4 : 10;
  const turnContribution = me.level === 1 ? 100 : me.level === 2 ? 175 : 275;

  lblMyLevelBadge.textContent = `LVL ${me.level} ${me.level === 1 ? 'STREET INVESTOR' : me.level === 2 ? 'PORTFOLIO MANAGER' : 'ASSET OWNER'}`;
  lblNextThreshold.textContent = me.level === 3
    ? `Mastery complete · +$${turnContribution}/turn`
    : `Progress: ${me.investorProgress}/${nextThreshold} · +$${turnContribution}/turn`;
  const currentFloor = me.level === 2 ? 4 : 0;
  const progressPercent = me.level === 3
    ? 100
    : Math.min(100, Math.max(0, ((me.investorProgress - currentFloor) / (nextThreshold - currentFloor)) * 100));
  ...
  for (const [sym, qty] of Object.entries(me.portfolio.stocks as Record<string, number>)) {
    ...
    const returnEl = document.getElementById(`val-return-${sym}`);
    if (sharesEl) sharesEl.textContent = `${qty} shares`;
    if (valEl) valEl.textContent = `$${val.toLocaleString()}`;
    if (returnEl) {
      const costBasis = me.portfolio.stockCostBasis?.[sym] || 0;
      const gainLoss = val - costBasis;
      const gainLossPercent = costBasis > 0 ? (gainLoss / costBasis) * 100 : 0;
      returnEl.className = `stock-return ${gainLoss > 0 ? 'positive' : gainLoss < 0 ? 'negative' : 'flat'}`;
      returnEl.textContent = qty > 0 && costBasis > 0
        ? `${gainLoss >= 0 ? '▲' : '▼'} $${Math.abs(Math.round(gainLoss)).toLocaleString()} (${gainLoss >= 0 ? '+' : ''}${gainLossPercent.toFixed(1)}%)`
        : '—';
      returnEl.title = qty > 0 && costBasis > 0
        ? `Average cost $${(costBasis / qty).toFixed(2)} per share`
        : 'No open position';
    }
  }
  ...
  if (propCount === 0) {
    ...
    me.portfolio.properties.forEach((_: any, idx: number) => {
      ...
      pRow.innerHTML = `
        ...
        <span>+$140 income · $80 maintenance</span>
      `;
...
function handlePopOutsAndModals(state: any, me: any) {
  ...
  if (pending?.type === 'decision' && pending.details.card) {
    ...
    if (!isMyTurn) return;
    const card = pending.details.card;
    const sourceLabel = pending.details.tileSource || card.category.replace('_', ' ');
    drawerActionBadge.textContent = `${sourceLabel.toUpperCase()} · LEVEL ${me.level}`;
    drawerBodyContent.innerHTML = `
      <div class="decision-card-intro">
        <span class="decision-concept">${card.concept}</span>
        ${card.socialMediaScenario ? `<span class="social-source-chip">SOCIAL SIGNAL · ${card.socialSource}</span>` : ''}
        <h2>${card.title}</h2>
        <p>${card.scenario}</p>
      </div>
      ${card.objective ? `
        <div class="decision-brief">
          <div><strong>YOUR OBJECTIVE</strong><span>${card.objective}</span></div>
          <div><strong>EVIDENCE</strong><ul>${(card.evidence || []).map((item: string) => `<li>${item}</li>`).join('')}</ul></div>
        </div>
      ` : ''}
      <div class="card-tip-callout ${card.gameOnlyInsiderMechanic ? 'insider-warning' : ''}">
        <strong>${card.gameOnlyInsiderMechanic ? '⚖ LEGAL DISCLAIMER' : '💡 INVESTOR TIP'}</strong>
        <span>${card.tip}</span>
      </div>
      ...
      <p class="decision-footnote">${card.category === 'decision' ? 'Strongest fit: +2 progress · Defensible: +1 · Bias trap: 0. ' : ''}Consider time horizon, valuation, diversification, liquidity and downside risk.${card.socialMediaScenario ? ' Your private influence pattern updates after this choice.' : ''}</p>
    `;
...
function handleIncomingGameState(gameState: any) {
  ...
  if (roll && roll.rollId !== lastProcessedRollId) {
    ...
    setTimeout(() => {
      ...
      const stateToRender = pendingGameState || gameState;
      renderGame(stateToRender);
      notifyGameMoments(stateToRender);
